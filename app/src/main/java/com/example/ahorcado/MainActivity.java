...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState == null){//Esta linea hace que si no existe se cree el codigo
            getSupportFragmentManager().beginTransaction().add(R.id.ventanajuego, new VentanaAhorcado()).commit();//Creador de fragmento. R= llama a todos los recursos de res
        }
    }
    ...
    private void chequeaLetra(String letra){
        ...
        for (int i=0; i<palabraOculta.length(); i++){
            if (palabraOculta.charAt(i)==letra.charAt(0)){
                //quita el guion de la letra correspondiente
                palabraConGuiones = palabraOculta.substring(0, 2*i)+letra+palabraConGuiones.substring(2*i+1);
                acierto = true;
            }
        }
        ...
        if(!acierto){
            ...
            switch (numeroDeFallos){
                ...
                case 6 : imagenAhorcado.setImageResource(R.drawable.ahorcado_fin); break;
